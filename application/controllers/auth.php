...
class auth extends CI_Controller
{
	...
	public function registration()
	{
		...
		$this->form_validation->set_rules('email', 'Email', 'required|trim|valid_email|is_unique[user.email]', [
			'is_unique' => 'This Email has available!!'
		]);
		...
		if ($this->form_validation->run() == false) {
			...
			$this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">
  			Create New User Succes! Please activation you account</div>');
			redirect('auth');
		}
	}
	...
	private function _sendEmail($token, $type)
	{
		$config = [
			...
			'smtp_user' => 'nurjuliadis@gmail.com',
			'smtp_pass' => 'Hada0707',
			'smtp_port' => 465,
			'mailtype' => 'html',
			'chartset' => 'utf-8',
		...
		$this->email->from('nurjuliadis@gmail.com', 'PBN Record System');
	...
	public function forgotPassword()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			if ($user) {
				...
				$this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">
  				Please check you email to reset password !</div>');
				redirect('auth/forgotpassword');
			} else {
				$this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">
  				Email / Account not registerd or activated !! Wrong email</div>');
				redirect('auth/forgotpassword');
			}
		}
	}
